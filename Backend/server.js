...
// NEW SCHEMA: For saving the grand totals from the table
...
// Routes for Reports
// Save a report (snapshot of customer data)
app.post('/api/reports', async (req, res) => {
    try {
        const reportData = req.body;
        const newReport = new Report(reportData);
...
// Get all saved customer reports
app.get('/api/reports', async (req, res) => {
    try {
        const reports = await Report.find();
        res.json(reports);
...
// NEW ROUTE: Save table totals
app.post('/api/table-totals', async (req, res) => {
    try {
        const tableTotalsData = req.body;
        // Create a new document using the TableTotalReport model
...
// OPTIONAL: NEW ROUTE: Get all saved table totals (if you need to retrieve them later)
