...
const Teesimage = () => {
  ...
  const findAvailableImages = async () => {
    ...
    while (consecutiveFails < batchSize && batchStart <= 1000) {
      ...
      for (let i = batchStart; i < batchStart + batchSize; i++) {
        batchPromises.push(checkImageExists(`/teesimages/image${i}.webp`));
      }
      ...
      for (let i = 0; i < batchResults.length; i++) {
        ...
        if (batchResults[i]) {
          foundImages.push(`/teesimages/image${currentIndex}.webp`);
  ...
  return (
    <div style={styles.container}>
      <h1>Tees Gallery ({imageCount} images)</h1>
